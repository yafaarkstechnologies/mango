        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {REVIEWS.map((review, i) => (
            <motion.div 
              ...
              className="relative p-12 rounded-[3rem] bg-white/[0.02] border border-white/5 flex flex-col justify-between hover:bg-white/[0.04] transition-all duration-500 group"
            >
              ...
              <div className="flex items-center gap-4 mt-12 pt-8 border-t border-white/5">
                ...
                <div className="flex flex-col">
                  <span className="text-white font-black uppercase text-sm tracking-widest">{review.name}</span>
                  <span className="text-white/30 text-xs uppercase tracking-widest font-bold font-mono">{review.location}</span>
                </div>
              </div>
            </motion.div>
          ))}
